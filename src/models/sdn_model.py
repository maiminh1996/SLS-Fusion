...
class SDNet(nn.Module):
    ...
    def forward(self, left, right, calib):
        ...
        print("left: ", left.type())
        print("right: ", right.type())
        print("calib ", calib.type())
        


        refimg_fea = self.feature_extraction(left)
        targetimg_fea = self.feature_extraction(right)
        print("refimg_fea: ", refimg_fea.type())
        print("targetimg_fea: ", targetimg_fea.type())
...
class SDNet(nn.Module):
    ...
    def forward(self, left, right, calib):
        ...
        # calculate feature for right image
        targetimg_fea = self.feature_extraction(right) # [1,32,64,128]
        ##############################

        ##### Put them into disparity cost volume #####
        # cost [bs, 32*2, 192/4, 64, 128] # [0, 191]: dispa pixel max
        cost = Variable(
            torch.cuda.FloatTensor(refimg_fea.size()[0], refimg_fea.size()[1] * 2, self.maxdisp // 4,
                                   refimg_fea.size()[2],
                                   refimg_fea.size()[3]).zero_())
        ...
        # # [1, 80]: depth grid
        cost = cost.contiguous() # [bs, 32*2, 192/4, 64, 128]
        cost = self.warp(cost, calib) # [bs, 32*2, 40, 64, 128]
        ##########################################

        ##### conv 3D #####
        cost0 = self.dres0(cost) # conv 3D # [bs, 32, 40, 64, 128]
