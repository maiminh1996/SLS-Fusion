...
class PSMNet(nn.Module):
    ...
    def forward(self, left, right, calib,  out_std=False, out_cost_volume=False):
        ...
        targetimg_fea = self.feature_extraction(right) # shared weight, using the same layer
        ...
        pred3_out = disparityregression(self.maxdisp)(pred3)

        if self.training:
            return pred1, pred2, pred3_out
        else:
            if out_std:
                std = disparityregression_std(self.maxdisp)(pred3, pred3_out)
