...
def convert_and_save(args, i):
    ...
    im.save(osp.join(args.output_path, "{:06d}.png".format(i)))
    # plt.savefig(osp.join(args.output_path, "{:06d}".format(i)), depth_map)
    # np.save(osp.join(args.output_path, "{:06d}".format(i)), depth_map)

if __name__ == "__main__":
    
    # python ptc2depthmap.py --output_path /media/zelt/Données/KITTI/3D_OD/training/production/mix_clear_multifog/velodyne_fog_64beams_png \
    # --input_path /media/zelt/Données/KITTI/3D_OD/training/production/mix_clear_multifog/velodyne_fog_64beams \
    # --calib_path /media/zelt/Données/KITTI/3D_OD/training/calib \
    # --image_path /media/zelt/Données/KITTI/3D_OD/training/production/mix_clear_multifog/image_2/image_2fog \
    ...
    # project velodyne 16 beams
    # python ptc2depthmap.py --output_path /media/zelt/Données/KITTI/3D_OD/training/production/production/velodyne_projected_16beams_left \
    # --input_path /media/zelt/Données/KITTI/3D_OD/training/velodyne_16beams \
    # --calib_path /media/zelt/Données/KITTI/3D_OD/training/calib \
    # --image_path /media/zelt/Données/KITTI/3D_OD/training/image_2 \
    # --split_file /media/zelt/Données/SLS-Fusion/dataset/kitti/ImageSets/trainval.txt  --threads 4
