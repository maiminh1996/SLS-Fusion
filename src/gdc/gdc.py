...
def GDC(pred_depth, gt_depth, calib,
    ...
    try:
        ...
        Apinv = np.linalg.pinv(As)
        save = []
        for i in range(bs.shape[0]):
            save.append(list(np.tensordot(Apinv[i, :, :], bs[i,:], axes=([-1],[0]))))
        # print("excep: {}, {}, {}".format(As.shape, bs.shape, np.array(save).shape))
        W = np.array(save)[:, :k]
        # pass
    ...
    #         g= np.linalg.lstsq(build_a(x_data), y_data)
    #         print(g.shape)
    #         return g
    #     print("exept numpy.linalg.LinAlgError")
    #     print(As.shape)
    #     print(bs.shape)
    #     W = lstsq(As, bs)
    #     # W = np.linalg.lstsq(As, bs)
    #     # W = np.linalg.lstsq(As, bs)[:, :k]
